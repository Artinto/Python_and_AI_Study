...
w = torch.tensor([1.0], requires_grad=True) # 값이 1.0인 벡터 텐서 w를 선언하고, 이 w라는 텐서에 자동미분을 하여 그 기울기(미분값)를 w.grad에 저장함
...
# Before training
print("Prediction (before training)",  4, forward(4).item())  # 기계 학습 전 4를 대입했을 때의 forward 값: 4 x 1.0 = 4.0
...
for epoch in range(10): # 10번 경사 하강 
    for x_val, y_val in zip(x_data, y_data):  # x_val, y_val에 각각 x_data, y_data의 쌍들을 집어넣음
        ...
        l.backward() # 3) Back propagation to update weights, loss함수를 이용하여 기울기 계산 --> 그 값이 w.grad에 저장됌
