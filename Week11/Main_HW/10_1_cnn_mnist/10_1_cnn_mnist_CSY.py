...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # 2d convolution 연산 수행/입력 채널 1개, 출력 채널 10개, 커널 사이즈 5
        # 2d convolution 연산 수행/입력 채널 10개, 출력 채널 20개, 커널 사이즈 5
        # kernal size가 2*2이고 이 안에서 제일 큰 수를 선택
        # convolution을 거친 뒤 앞 계층의 출력 크기인 320을 입력으로 설정하여 신경망 계층 생성(fully-connected layer)
...
def test():
    ...
    for data, target in test_loader:
        ...
        output = model(data)
        # sum up batch loss
        # 배치 손실값의 합/size_average=False -> loss를 발생시키는 것을 평균으로 나누지 않음
