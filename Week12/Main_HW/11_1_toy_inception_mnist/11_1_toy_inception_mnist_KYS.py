...
# autograd를 사용하면 backprop을 위한 미분 값을 자동으로 계산
# 자동 계산을 위해 사용하는 변수가 Variable
...
class InceptionA(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.branch5x5_2 = nn.Conv2d(16, 24, kernel_size=5, padding=2) # padding 적용
    ...
    def forward(self, x):
        ...
        branch_pool = F.avg_pool2d(x, kernel_size=3, stride=1, padding=1) # avg pooling(입력에 대한 평균 pooling 수행)
        branch_pool = self.branch_pool(branch_pool)

        outputs = [branch1x1, branch5x5, branch3x3dbl, branch_pool] # 결과들을 하나로 합친다
...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(1408, 10) # define fully connect layer (input = 1408, output = 10)

    def forward(self, x):
        in_size = x.size(0)
        x = F.relu(self.mp(self.conv1(x))) # conv1(x)의 결과값을 maxpolling하고 relu 진행
        x = self.incept1(x) # inception layer 생성
...
def test(): # start test
    ...
    for data, target in test_loader:
        ...
        correct += pred.eq(target.data.view_as(pred)).cpu().sum() # target data와 pred 비교해서 동일한지 판단
