...
x_data = tensor([[1.0], [2.0], [3.0], [4.0]]) # input tensor
y_data = tensor([[0.], [0.], [1.], [1.]])     # output tensor as binary
...
# 이진 분류에 특화된 loss 함수
# reduction='mean' == 결과값의 평균
criterion = nn.BCELoss(reduction='mean')

# optimizer setting
# 경사 하강법인 SGD를 사용하고 learning rate는 0.01로 설정
# model.parameters()는 w(weight)와 b(bias)를 전달
optimizer = optim.SGD(model.parameters(), lr=0.01)

# Training loop
for epoch in range(1000): # == for(i = 0; i < 1000; i++)
    # Forward pass: Compute predicted y by passing x to the model
    y_pred = model(x_data) # 예측값 저장

    # Compute and print loss
    # 예측값(y_pred)과 결과값(y_data)의 loss를 구함
...
hour_var = model(tensor([[1.0]])) # 학습 후 1.0을 입력 해 결과값 도출
print(f'Prediction after 1 hour of training: {hour_var.item():.4f} | Above 50%: {hour_var.item() > 0.5}')
hour_var = model(tensor([[7.0]])) # 학습 후 7.0을 입력 해 결과값 도출
